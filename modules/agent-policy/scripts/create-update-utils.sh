...
get_formatted_list_of_strings() {
    local formatted="$(echo $1 | python -c 'import json, sys; \
...
function get_formatted_list_of_list_of_objects() {
    local formatted="$(echo $1 | python -c 'import json, sys; \
        list_of_list_of_objs = json.load(sys.stdin); \
        print (";".join(",".join("=".join(inner_list[x] for x in inner_list) \
            for inner_list in outer_list) for outer_list in list_of_list_of_objs))')"
    echo "$formatted"
}
...
function get_update_flags() {
    ...
    local clear_group_labels_flag="$(get_update_flag "$group_labels_flag_name" \
        "$group_labels_flag_value")"
    local clear_zones_flag=$(get_update_flag "$zones_flag_name" "$zones_flag_value")
    local clear_instances_flag=$(get_update_flag "$instances_flag_name" "$instances_flag_value")
...
#   $2 = project id
#   $3 = policy id
#   $4 = description of the agent policy
#   $5 = agent rules, in json format
#   $6 = group labels, in json format
#   $7 = os types, in json format
#   $8 = zones, in json format
#   $9 = instances, in json format
# Return:
#   the appropriate gcloud create or update command, given the args
function get_upsert_command() {
    local action="$1"
    local project_id="$2"
    local policy_id="$3"
    local description="$4"
    local agent_rules_json="$5"
    local group_labels_json="$6"
    local os_types_json="$7"
    local zones_json="$8"
    local instances_json="$9"
    ...
    local agent_rules_flag_value=$(get_formatted_list_of_objects "$agent_rules_json")
    local group_labels_flag_value=$(get_formatted_list_of_list_of_objects "$group_labels_json")
    local os_types_flag_value=$(get_formatted_list_of_objects "$os_types_json")
    local zones_flag_value=$(get_formatted_list_of_strings "$zones_json")
    local instances_flag_value=$(get_formatted_list_of_strings "$instances_json")

    local description_flag=$(get_flag "$description_flag_name" "$description")
    local agent_rules_flag=$(get_flag "$agent_rules_flag_name" "$agent_rules_flag_value")
    local group_labels_flag=$(get_flag "$group_labels_flag_name" "$group_labels_flag_value")
    local os_types_flag=$(get_flag "$os_types_flag_name" "$os_types_flag_value")
    local zones_flag=$(get_flag "$zones_flag_name" "$zones_flag_value")
    local instances_flag=$(get_flag "$instances_flag_name" "$instances_flag_value")
    local project_flag=$(get_flag "$project_flag_name" "$project_id")
    ...
    command=""$command""$os_types_flag""$zones_flag""$instances_flag""$update_flags""$project_flag""
    echo "$command"
}
...
function get_describe_command() {
    ...
    command=""$command" "$policy_id""$project_flag""
    echo "$command"
}